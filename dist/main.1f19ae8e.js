...
var firstDiv = cathy.create("<div>\u6211\u662F\u7B2C\u4E00\u4E2Adiv</div>"); //新建的节点如果不做其它操作是不会体现在网页上的

cathy.after(newSib, firstDiv); //这样子插入就体现在网页上来
//所以是可以直接用元素的ID名字test指代那个元素的

cathy.before(newSib, firstDiv);
var newSon = cathy.create("<div>\u6211\u662F\u7B2C\u4E00\u4E2Adiv\u7684\u513F\u5B50</div>");
cathy.append(firstDiv, newSon);
var daddy = cathy.create("<div>\u6211\u662F\u7B2C\u4E00\u4E2Adiv\u7684daddy</div>");
cathy.wrap(firstDiv, daddy); //=================以上为增====================
...
cathy.on(eventTest, 'click', fn); // cathy.off(eventTest,'click', fn)
//=================以上为改====================

var t = cathy.find('#find');
console.log(t); //.find也就是querySelectorAll(selector)找到的是包含div#find的NodeList,在页面上选择不到

console.log(Array.from(t)); // console.log(t[0])//加上下标[0]之后才是div#find，才在页面上选择得到
